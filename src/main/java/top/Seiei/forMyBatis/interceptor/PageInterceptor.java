...
import top.Seiei.forMyBatis.pojo.Page;
...
public class PageInterceptor implements Interceptor{
	...
	// 判断是否为要拦截的对象，参数是被检测的对象
	// 如果不是要拦截的对象，则返回，如果是，则返回达成协议，具有代理权力的代理类
	// 这里就是拦截要触发 prepare 方法的对象，增删改查都需要这个方法，所以这里只是初步过滤
	...
	// 参数 invocation 为被拦截下来的对象
	// 首先，需要进一步筛选，比如分页功能只是查，就要把增删改这些给过滤掉
	// 如何过滤？ invocation 是被拦截的对象，使用反射就可以获取它的一切信息
    // 此时只要命名需要分页功能的查询方法名称加入特定的识别字母（如 ByPage 字母作后缀）,进行筛选即可
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		MappedStatement mappedStatement = (MappedStatement) metaObject.getValue("delegate.MappedStatement"); // 字符串是要获取信息的 OGNL 表达式
